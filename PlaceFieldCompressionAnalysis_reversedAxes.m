...
% ids = [126,127,140,93,78,81,168,166,151,149]; % 79, 80
...
for iii = 1:length(conditions)
    ...
    plot(y1,x,'.'), hold on
    P = polyfit(y1,x,1); yfit = P(1)*y1+P(2);
    [R,P] = corrcoef(y1,x);
    text(90,60,['R = ' num2str(R(2,1),3),', P = ', num2str(P(2,1),3)],'Color','k')
    ylabel('Distance between fields (cm)'), xlabel('Time lag between theta spiking (ms)'), title(['Compression: ' conditions{iii}]), 
    ylim([-100,100]), xlim([-150,150]), grid on, hold on
%     [slope1,~,~] = CircularLinearRegression(y1/10,x,1);
%     plot(x,2*pi*slope1*x*10,'k-','linewidth',1.5), text(50,-100,num2str(slope1))
    slope2 = fitEllipse([y1;x]'); axis tight, text(50,-70,['Ellipse: ' num2str(slope2)])
    ...
    P = polyfit(y1,x,1); yfit = P(1)*y1+P(2);
    [R,P] = corrcoef(y1,x);
    text(0.130,-0.8,['R = ' num2str(R(2,1),3),', P = ', num2str(P(2,1),3)],'Color','k')
    ylabel('Time between fields (s)'), xlabel('Time lag between theta spiking (s)')
    ylim([-0.90,0.90]), xlim([-0.150,0.150]), grid on, hold on
...
%     plot(x,2*pi*slope1*x,'r-','linewidth',1.5), text(50,-5,num2str(slope1))
    slope2 = fitEllipse([y1;x]'); axis tight, text(-70,0.50,['Ellipse: ' num2str(slope2)])
    ...
    P = polyfit(y1,x,1); yfit = P(1)*y1+P(2);
    [R,P] = corrcoef(y1,x);
    text(7.130,-50,['R = ' num2str(R(2,1),3),', P = ', num2str(P(2,1),3)],'Color','k')
    ylabel('Distance between fields (cm)'), xlabel('Phase lag between fields')
    ylim([-100,100]), xlim([-2*pi,4*pi]), grid on, hold on
    [slope1,~,~] = CircularLinearRegression(x,y1,1);
    plot(y1,2*pi*slope1*y1,'r-','linewidth',1.5), text(-3.,20,['Phase precession: ',num2str(slope1)]), axis tight
    slope2 = fitEllipse([y1;x]'); axis tight, text(-4,20,['Ellipse: ' num2str(slope2)])
    
    figure(2)
    subplot(2,2,3), hold on
    [param]=sigm_fit(y1,x);
    text(5,-80+iii*40,['Slope: ' num2str(param(4),3)],'Color','k')
    figure(4)
...
%     histogram(y1,[-0.15:0.01:0.15],'Normalization','probability'), grid on, axis tight, legend(conditions)
    ...
    x1 = placefield_speed{iii}; 
    x = placefield_difference{iii}; 
    y1 = ccg_delay{iii};
    subset = find( y1 < 90 & y1 > -90 & x > -60 & x < 60  & precession_slope{iii}(1,:)<-0.005 & precession_slope{iii}(1,:)>-0.03 & precession_slope{iii}(2,:)<-0.005 & precession_slope{iii}(2,:)>-0.03 );
    x1 = x1(subset); y1 = abs(y1(subset));
    plot(y1,x1,'.','Color',colors{iii})
    P = polyfit(y1,x1,1); yfit = P(1)*y1+P(2);
    plot(y1,yfit,colors{iii})
    title([conditions{iii}, ' time']), ylabel('Animal speed'), xlabel('Tau')
    ...
    P = polyfit(y1,x1,1); yfit = P(1)*y1+P(2);
    plot(y1,yfit,colors{iii})
    title([conditions{iii},' phase']), ylabel('Animal speed'), xlabel('Tau')
end
figure
x1 = diff(vertcat(placefield_speed{:}));
y1 = diff(abs(vertcat(placefield_difference{:})));
subset = find(y1<50 & y1>-50);
x1 = x1(subset);
y1 = y1(subset);
plot(y1,x1,'.'), hold on
P = polyfit(y1,x1,1); yfit = P(1)*y1+P(2);
plot(y1,yfit,'k')
title('Slope'), ylabel('Animal speed difference'), xlabel('Tau difference')
...
for iii = 1:length(conditions)
    ...
    plot(circ,linc,'.k')
    ylabel('Distance between fields (cm)'), xlabel('Time lag between theta spiking (ms)'), title(['Compression: ' conditions{iii}]), 
    ylim([-100,100]), xlim([-490,490]), grid on, hold on,% axis tight
...
%     UpperLimits = [   8,  200, 100, 1500,    0.01,    0.1,   100];
    StartPoint =  [   0.5,  100,  30,  500, 0.00001,   0.03,    0];
    LowerLimits = [ 1/8,   50,  10,  200,       0,      0,  -100];
    UpperLimits = [   2,  200, 100, 1500,    0.01,    0.1,   100];
    [fit_params,f01] = customFit2(x_out,y_out,x_bins,y_bins,StartPoint,LowerLimits,UpperLimits);
    ...
    subplot(3,3,1+iii-1), hold on 
    for i = -5:5
        plot(x_bins/fit_params(1)+i*fit_params(2),x_bins,'--r')
    end
    xlim([-200,200]),ylim([-40,40])
    figure(20)
    subplot(1,3,1)
    for i = -5:5
        plot(x_bins/fit_params(1)+i*fit_params(2),x_bins,colors{iii}), hold on
    end
    xlim([-200,200]),ylim([-40,40]),title('Distance')
    ...
    plot(circ,linc,'.k')
    ylabel('Time between fields (s)'), xlabel('Time lag between theta spiking (s)')
    ylim([-650,650]), xlim([-490,490]), grid on, %axis tight
%     slope2 = fitEllipse([linc;circ]'); 
    axis tight, 
...
%     UpperLimits = [0.3,  150,  700,  1500,    0.001,  1.5,   500];
    StartPoint =  [5,   100,  400,   800,  0.00001,  0.6,     0];
    LowerLimits = [1,   50,  100,   400, 0.000001, 0.2,  -500];
    UpperLimits = [10,  150,  700,  1500,    0.001,  1.5,   500];
%     pd = fitdist([x_out;y_out],'SpatialTemporalCompression')
    [fit_params,f02] = customFit2(x_out,y_out,x_bins,y_bins,StartPoint,LowerLimits,UpperLimits);
    ...
    xlim([-200,200]),ylim([-400,400]),title('Time')
    ...
    plot(circ,linc,'.k')
    ylabel('Distance between fields (cm)'), xlabel('Phase lag between fields')
    ylim([-50,50]), xlim([-10*pi,10*pi]), grid on, %axis tight
    indx = find( linc > -10 & linc < 10);
...
%     UpperLimits = [1,10,200,200,0.01,1,50];
    StartPoint = [10,6.28,40,20,0.00001,0,0];
    LowerLimits = [0,0,10,5,0,-1,-50];
    UpperLimits = [20,10,200,200,0.01,1,50];
    [fit_params,f03] = customFit2(x_out,y_out,x_bins,y_bins,StartPoint,LowerLimits,UpperLimits);
    ...
    xlim([-10,10]),ylim([-40,40])
    figure(20)
    subplot(1,3,3)
    for i = -5:5
        plot(x_bins/fit_params(1)+i*fit_params(2),x_bins,colors{iii}), hold on
    end
    xlim([-10,10]),ylim([-40,40]),title('Phase')
    ...
    xticks([-8*pi:2*pi:8*pi]),xticklabels({'-8\pi','-6\pi','-4\pi','-2\pi','0','2\pi','4\pi','6\pi','8\pi'})
    subplot(3,2,6), hold on
    x_interval = 20;
...
for i = 1:7
    ...
    plot([1,2],mean(data'),'ok','linewidth',2);
    plot(data,'k')
