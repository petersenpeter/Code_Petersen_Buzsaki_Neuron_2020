...
shank_nb = 1;
for shank = 1:shanks
    spike_times = double(hdf5read([folder,'/', dataset, '.kwik'], ['/shanks/shank' num2str(shank-1) '/spikes/time']));
    %recording_nb = double(hdf5read([folder,'/', dataset, '.kwik'], ['/channel_groups/' num2str(shank-1) '/spikes/recording']));
    cluster_index = double(hdf5read([folder,'/', dataset, '.kwik'], ['/shanks/shank' num2str(shank-1) '/clusters/clusters']));
    ...
    for i = 1:length(clusters(:))
        cluster_type = double(hdf5read([folder,'/', dataset, '.kwik'], ['/shanks/shank' num2str(shank-1) '/groups_of_clusters' '/channel_groups/'));
        if cluster_type == 2
