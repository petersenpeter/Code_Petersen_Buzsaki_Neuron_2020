...
%         for li1 = 1 : size(segspikes,2)
            segburstivs = find(tivs <= max(ibclust));
            sbiv1 = diff(segburstivs);
            sbiv2 = find(sbiv1 > 1);
            firstspikes = segspikes(segburstivs(sbiv2 + 1));
            lastspikes = segspikes(segburstivs(sbiv2) + 1);
            firstspikes = [segspikes(1) firstspikes];
            lastspikes = [lastspikes segspikes(end)];
%         end
