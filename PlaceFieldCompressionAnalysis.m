...
%     histogram(y1,[-0.15:0.01:0.15],'Normalization','probability'), grid on, axis tight, legend(conditions)
    ...
    plot(x1,y1,'.','Color',colors{iii})
    P = polyfit(x1,y1,1); yfit = P(1)*x1+P(2);
    plot(x1,yfit,colors{iii})
    title([conditions{iii}, ' time']), xlabel('Animal speed'), ylabel('Tau')
    ...
    P = polyfit(x1,y1,1); yfit = P(1)*x1+P(2);
    plot(x1,yfit,colors{iii})
    title([conditions{iii},' phase']), xlabel('Animal speed'), ylabel('Tau')
end
figure
...
P = polyfit(x1,y1,1); yfit = P(1)*x1+P(2);
plot(x1,yfit,'k')
title('Slope'), xlabel('Animal speed difference'), ylabel('Tau difference')
...
for i = 1:length(idsToLoad)
    temp = load(['Z:\peterp03\IntanData\Analysis\PlaceFieldCompressionAnalysis_',num2str(idsToLoad(i)),'.mat']);
    fit_params_all11 = [fit_params_all11,temp.fit_params_all11(:,[1,4,7])];
    fit_params_all22 = [fit_params_all22,temp.fit_params_all22(:,[1,4,7])];
    fit_params_all33 = [fit_params_all33,temp.fit_params_all33(:,[1,4,7])];
end

fit_xticklabels = {'Slope','Y period','X envelope','Y envelope','Amplitude','xy shift','Y offset'};
colors2 = {'r','b','g'};
idx = [1:3:length(idsToLoad)*3];
figure
for i = 1:7
    subplot(3,7,i)
    for j = 1:length(conditions)
    ...
    axis tight
    data = reshape(fit_params_all11(i,:),3,[]);
    plot(data,'k')
    [h1,p1] = signrank(data(1,:),data(2,:));
    [h2,p2] = signrank(data(2,:),data(3,:));
    [h3,p3] = signrank(data(1,:),data(3,:));
    temp = get(gca,'ylim');
    h1 = text(1.1,temp(2),['h=', num2str(h1),',p=',num2str(p1)])
    h2 = text(2.7,temp(2),['h=', num2str(h2),',p=',num2str(p2)])
    h3 = text(2.2,temp(1),['h=', num2str(h3),',p=',num2str(p3)])
    set(h1,'Rotation',30); set(h2,'Rotation',30); set(h3,'Rotation',30);
end
...
for i = 1:7
    for j = 1:length(conditions)
        ...
        axis tight
    data = reshape(fit_params_all22(i,:),3,[]);
    plot(data,'k')
    [h1,p1] = signrank(data(1,:),data(2,:));
    [h2,p2] = signrank(data(2,:),data(3,:));
    [h3,p3] = signrank(data(1,:),data(3,:));
...
for i = 1:7
    for j = 1:length(conditions)
        ...
        axis tight
    data = reshape(fit_params_all33(i,:),3,[]);
    plot(data,'k')
    [h1,p1] = signrank(data(1,:),data(2,:));
    [h2,p2] = signrank(data(2,:),data(3,:));
    [h3,p3] = signrank(data(1,:),data(3,:));
