...
ch_wheel_pos = recordings(id).wheel_pos; % Wheel channel (base 1)
ch_temp = recordings(id).temp; % Temperature data included (base 1)
ch_peltier_voltage = recordings(id).peltier_voltage; % Peltier channel (base 1)
ch_fan_speed = recordings(id).fan_speed; % Fan speed channel (base 1)
...
for j = 1:nb_lfp_periods
    j
    %start = cooling(i,1);
    %duration = cooling(i,2)-cooling(i,1);
    start = time_frame(1)+lfp_periods*(j-1);
    lfp_wheel_temp = 0.000050354 * double(LoadBinary(fname,'nChannels',nbChan,'channels',ch_lfp,'precision','int16','frequency',sr,'start',start,'duration',min(lfp_periods,time_frame(2)-time_frame(1)-(j-1)*lfp_periods)));
    lfp_filt = filtfilt(b1,a1,lfp_wheel_temp);
    lfp_wheel{j} = downsample(lfp_filt,downsample_n1);
    [wt,~,~] = awt_freqlist(lfp_wheel{j},sr_lfp,freqlist);
    lfp_wavelets{j} = abs(wt)';
end
fname_digi = [datapath, recording, '/digitalin.dat'];
[digital_on,digital_off] = Process_IntanDigitalChannels(fname_digi);
camera_delay = min(digital_on{ch_camera_sync}(1),digital_off{ch_camera_sync}(1))/sr;
...
vidObj = VideoReader(tracking_data_filebase);
...
gausswin_width = floor(vidObj.FrameRate)*15;
video_frame_diff2 = zscore(nanconv(video_frame_diff,gausswin(gausswin_width)'/sum(gausswin(gausswin_width)),'edge'));
...
xlim([0,3])
subplot(2,1,1)
time_axis = digital_off{ch_camera_sync}(1:length(video_frame_diff2))/sr; % (1:length(video_frame_diff2))/vidObj.FrameRate;
...
subplot(2,1,1)
test2 = video_frame_diff2(gausswin_width:end-gausswin_width)<video_threshold;
% test(find(test2<100))= [];
% test(find(test>length(video_frame_diff2)-gausswin_width/3))= [];
quiet = [];
quiet(:,1) = find(diff(test2) == 1)+gausswin_width;
quiet(:,2) = find(diff(test2) == -1)+gausswin_width;
quiet = [quiet(1:end-1,1),quiet(2:end,2)];
sleep_min_time = 10; % in seconds
...
StateEditor_motion_from_camera = [zeros(1,round(camera_delay)), motion_from_camera2,zeros(1,recording_length-round(camera_delay)-length(motion_from_camera2))];
...
downsample_n2 = 200;
% wheel_pos = downsample(v(ch_wheel_pos,:),downsample_n2);
v_downsample = mean(reshape(v(ch_temp,1:end-mod(size(v,2),downsample_n2)),downsample_n2,[])); clear v;
v_downsample(v_downsample<1.25) = 1.25;
temperature = nanconv((v_downsample-1.25)/0.005,ones(1,100)/100,'edge');
...
StateEditor_Temperature_Motion = [StateEditor_Temperature;StateEditor_motion_from_camera];
save([datapath recording '/StateEditor_Temperature_Motion.mat'],'StateEditor_Temperature_Motion')
...
%%
load([datapath recording '/' recording '-states.mat'])
load([datapath recording '/StateEditor_Temperature.mat'],'StateEditor_Temperature')
load([datapath recording '/StateEditor_Theta.mat'],'StateEditor_theta_periods')
REM_states = find(states == 5 & StateEditor_theta_periods > 4);
x1 = StateEditor_Temperature(REM_states);
y1= StateEditor_theta_periods(REM_states);
figure
plot(x1,y1,'o'), hold on
% plot(x1+0.5,y1,'ob')
P = polyfit(x1,y1,1);
yfit = P(1)*x1+P(2);
plot(x1,yfit,'k-','linewidth',2);
% text(26,7.5,['REM theta slope: ', num2str(P(1),3)],'Color','blue'), axis tight
[r,p] = corr(x1',y1')
ylabel('Theta frequency (Hz)'), title(['REM theta, slope: ', num2str(P(1),3) ' (Hz/C)']), xlabel('Brain temperature (C)')