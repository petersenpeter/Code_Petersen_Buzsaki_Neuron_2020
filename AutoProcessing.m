...
AquisitionDataPath = 'X:\IntanData';
SSDPath = 'G:\Kilosort';
ids = [138]; % 127,
t_auto = tic;
Timer = [];

StepsToPerform = [2]; % [1,2,3,4,6,7,8];%1:10
% Steps:
...
for i = 1:length(ids)
    ...
    AnimalPath = ['Peter_',recording.animal_id];
    
    % Copying recording folders from PeterSetup if local folder is not existing already
    if any(StepsToPerform == 1)
        disp('Step 1: Copying recording folders from PeterSetup and renaming files')
    ...
    if any(StepsToPerform == 4)
        ...
        Timer.event{4} = 'Creating eeg file';
    end
