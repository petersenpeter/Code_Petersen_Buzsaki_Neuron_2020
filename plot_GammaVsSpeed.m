function plot_GammaVsSpeed(recording,animal,cooling)
disp('Plotting Theta vs running speed')
bins_speed = [5:3:150]; % [-40:5:40]; % [5:3:60]
bins_speed_avg = mean([bins_speed(2:end);bins_speed(1:end-1)]);
freqlist = 10.^(0.6:0.01:2); %[4:0.5:55];
...
wt2 = [zeros(length(freqlist),124),abs(wt), zeros(length(freqlist),125)]; clear wt
...
subplot(2,3,4)
stairs(freqlist,nanmean(wt_speed_cooling),'b'), hold on
stairs(freqlist,nanmean(wt_speed_nocooling),'r'), hold on
